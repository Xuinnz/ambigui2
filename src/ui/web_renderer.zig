...
const COL_BLACK = rl.Color{ .r = 0, .g = 0, .b = 0, .a = 255 };
...
fn loadBgFrames() void {
    // filesystem unavailable on WASM — bg animation disabled
    // emcc --preload-file handles asset bundling at link time
}

fn loadTex(path: [:0]const u8) ?rl.Texture2D {
    const t = rl.LoadTexture(path);
    if (t.id == 0) return null;
    rl.SetTextureFilter(t, rl.TEXTURE_FILTER_BILINEAR);
    return t;
}
...
pub fn preloadAssets() void {
    // no loading screen on WASM — just load directly
...
fn drawFullscreenTex(tex: rl.Texture2D) void {
    ...
    rl.DrawTexturePro(tex, src, dest, rl.Vector2{ .x = 0, .y = 0 }, 0, rl.WHITE);
}
...
fn drawGhostPieces(layout: BoardLayout, state: *const GameState) void {
    ...
    drawPieceOutlineOnBoard(layout.board_x, &ghost_a, withAlpha(shapeColor(ghost_a.shape_type), 180));
...
fn drawProbBoxes(layout: BoardLayout, state: *const GameState) void {
    ...
    rl.DrawText(ta, layout.left_x + @divTrunc(BOX - wa, 2), PROB_A_Y + 2, 12, COL_WHITE);
    ...
    rl.DrawText(tb, layout.left_x + @divTrunc(BOX - wb, 2), PROB_B_Y + 2, 12, COL_WHITE);
}
